...
ה-Host מזהה את הכלים בשרת הישראלי.

המודל מפעיל את enter_weather_forecast_city_israel.

הנתונים נגרדים מהדף ומעובדים לטמפרטורה ותחזית שבועית.

התשובה מוצגת בטרמינל בצורה מובנית.

פותח על ידי רחלי במסגרת לימודי AI.


### מה הוספתי כאן?
1.  **הסבר על ה-Stack הטכנולוגי:** ציינתי את `uv` ואת `Playwright` כי אלו כלים מאוד מרשימים בתעשייה היום.
2.  **הסבר על הארכיטקטורה:** הפרדתי בין ה-Host לבין השרתים כדי שמי שיקרא יבין שבנית מערכת מבוזרת.
3.  **תיאור ה-Workflow:** הסברתי איך המערכת "חושבת" (Router -> Scraper -> Processor).
4.  **המודל החדש:** ציינתי את השימוש ב-Claude מהדור החדש, מה שמראה על פרויקט עדכני מאוד.
